...
- `task-create` 负责创建或更新任务、`prd.md` 和任务元数据
...
- `brainstorm` 在收口时应该主动询问：继续补充，还是进入 `task-create`
- `task-create` 负责把确认后的需求写入任务目录、`prd.md` 和 `task.json.dev_type`
- `task-start` 才会激活任务并启动实现子代理
- `task-start` 会在缺少 `implement.jsonl` 时，按 `task.json.dev_type` 自动补齐
